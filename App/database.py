...
class ORF(Base):
    ...
    ORF_index=Column(Integer) #ORF start codon 'M' index position on TRANSLATED file
...
# new_file=raw_file(id=1, file_name='MANUAL_INPUT_1', header='MANUAL_HEADER_INPUT')
# session.add(new_file)
# session.commit()

# query=session.query(raw_file.id>=0).all()
